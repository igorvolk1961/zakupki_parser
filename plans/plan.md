...
- ☐ Регистрация: email + пароль, статус `trial`, `trial_end_date=+10 лет` (подтверждение
      email — целевая модель, в MVP не реализуется; оплата/подписка не обязательны).
...
- ☐ 5A. Кэш ЭТП — Redis (решение принято): ключ `platform + okpd2-фильтр + страница`
...
- ☐ 5C. Очередь `parser:jobs` — RabbitMQ (решение принято) + stateless воркеры парсера;
