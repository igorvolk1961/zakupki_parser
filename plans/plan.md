...
## Этап 4. Кэш ЭТП и параллельная обработка площадок (R4, R5) — **пересмотр: 4B в MVP; 4A — пост-MVP**
- ✅ 4B. Параллельные площадки: asyncio-задачи в планировщике + `max_concurrent_platforms`
      (def: 2; 1 — последовательно). Опция `profile_then_platform` удалена — изоляция
      обхода по профилю через `deduplicate_requests: false` (ADR-11).
- ☐ 4A. Кэш ЭТП — **перенесён в пост-MVP** (ADR-11): в дефолтном `platform_then_profile`
      одинаковые обходы уже дедуплицируются `_build_units` за цикл, а TTL `0.5×period`
      < period (кэш не переживает цикл); ценность появится при повторе запросов между
      воркерами (4C/10). Redis-кэш: ключ `platform + okpd2-фильтр + страница`,
      TTL = `timeout_seconds × 0.5`, graceful degradation — при реализации 4C/10.
...
- ✅ Тесты: параллельный запуск (лимит `max_concurrent_platforms`), изоляция сбоя
      площадки, `=1` сохраняет последовательность; отсутствие задач без профилей/площадок.
...
- ✅ Формализованные проверки в `analysis_service`: опыт (ПП 2571, hard/soft/none),
...
- ✅ Явный статус «анализ отложен/недоступен» (NFR-FT-2): вердикт `unavailable` (⚪,
      «не проверено», severity 0) + верхнеуровневый `status` (`ok|deferred|error|no_tz`)
      в `rag_report`; сбой batch-LLM/эмбеддингов/верификации больше НЕ деградирует в 🟢.
      UI: баннер при `status == deferred`, серые пилюли «не проверено».
- ✅ Тесты: «не установлено» (AC US-4.3), hard/soft опыт (US-4.2), лицензии (US-4.4),
      маркеры, матчер, лексический ретривал, сбои → `unavailable`/`deferred`
...
## Текущий фокус
- **MVP выполнен (Этапы 0–5):** 0 (базовая линия/док), 1 (данные/изоляция BR-07),
  2 (профили + сид keywords), 3 (парсинг по ОКПД2 + клиентская фильтрация слов, R9),
  4 (R5 параллельные площадки; кэш 4A — пост-MVP), 5 (маркеры ТЗ + явный статус
  «анализ отложен», NFR-FT-2). Детальные планы — `plans/01..05_plan.md`.
...
- **Дальше — пост-MVP:** Этапы 6 (жизненный цикл аккаунта), 4C (RabbitMQ+воркеры),
  7 (решения/обратная связь), 8 (доставка/экспорт), 9 (compliance), 10 (наблюдаемость);
  кэш 4A — вместе с 4C/10.
