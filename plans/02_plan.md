...
`docs/references/profile.md` (R8) и сидом для профиля по умолчанию (сервис-аккаунт + каждый новый
...
### 2.1. Парсер `docs/references/profile.md` (`storage/keywords_parser.py`)
...
- Путь к файлу: `docs/references/profile.md` (относительно корня репозитория; env-оверрайд
...
- Регистрация и `_ensure_service_account()`: сид default-профиля с ключевыми словами
  из `docs/references/profile.md` (R8) вместо пустого профиля.
...
## Критерии приёмки
1. `docs/references/profile.md` разбирается в keywords/exclusion_words (секции, формы).
