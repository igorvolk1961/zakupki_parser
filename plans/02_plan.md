...
`data/key_words.md` (R8) и сидом для профиля по умолчанию (сервис-аккаунт + каждый новый
пользователь), синхронизацией JSONB↔keywords. Поведение парсера до Этапа 3 не меняется
(использует активный профиль сервис-аккаунта).

## Задачи (по порядку)

### 2.1. Парсер `data/key_words.md` (`storage/keywords_parser.py`)
...
- Путь к файлу: `data/key_words.md` (относительно корня репозитория; env-оверрайд
...
- Регистрация и `_ensure_service_account()`: сид default-профиля с ключевыми словами
  из `data/key_words.md` (R8) вместо пустого профиля.
...
### 2.4. Тесты
- Unit: парсер `key_words.md` (секции, формы `слов*`, `(…)~N`, кавычки, минус-слова).
...
## Критерии приёмки
1. `data/key_words.md` разбирается в keywords/exclusion_words (секции, формы).
