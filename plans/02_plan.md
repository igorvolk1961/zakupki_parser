...
`файл-сид профиля` (R8) и сидом для профиля по умолчанию (сервис-аккаунт + каждый новый
...
### 2.1. Парсер `файл-сид профиля` (`storage/keywords_parser.py`)
...
- Путь к файлу: `файл-сид профиля` (относительно корня репозитория; env-оверрайд
...
- Регистрация и `_ensure_service_account()`: сид default-профиля с ключевыми словами
  из `файл-сид профиля` (R8) вместо пустого профиля.
...
## Критерии приёмки
1. `файл-сид профиля` разбирается в keywords/exclusion_words (секции, формы).
