...
6) **Неоднозначность продукта.** Если в описании закупки НЕ названо конкретное ПО и неясно,
   ...
   **ВАЖНО: уточнение по полному тексту ТЗ — дорогая операция, не злоупотребляй ей.**
   requires_tz_review=true стави ТОЛЬКО если одновременно:
   a) есть РЕАЛЬНАЯ неоднозначность продукта (ПО не названо, текст двусмыслен);
   б) закупка в остальном ПЛАУЗИБЕЛЬНА для компании — скор не очевидно низкий (>=4)
...
- tz_review_necessity: нужно ли уточнять скор по полному тексту ТЗ и почему (реальная ли
...
FEW_SHOT: list[tuple[list[BaseMessage], list[BaseMessage]]] = [
    ...
    _ex(
        ...
        5.0,
        {
            ...
            "tz_review_necessity": "ПО не названо, скор средний (5) — есть реальная неоднозначность, уточнение по ТЗ оправдано",
