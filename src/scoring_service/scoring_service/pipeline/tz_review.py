"""Поиск и извлечение текста технического задания (requires_tz_review).

Когда fit-цепочка вернула ``requires_tz_review=True``, модуль ищет файл ТЗ
в карточке закупки и возвращает его очищенный текст для повторного
fit/judge-анализа (вместо описания закупки).
...
скор остаётся без изменений (уточнение не выполняется).
"""
