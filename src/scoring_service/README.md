...
диагностическая: на score не влияет, результат пишется в БД парсера как `embedding_similarity`
и в метаданные LangFuse-трейса. Без ключа доступа ветка не выполняется (факт пропуска
