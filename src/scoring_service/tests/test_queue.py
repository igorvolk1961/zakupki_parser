...
async def test_pop_job_returns_highest_priority(queue) -> None:
    await queue.enqueue(1, 10.0, profile_id=1)
    await queue.enqueue(2, 200.0, profile_id=2)
    await queue.enqueue(3, 50.0, profile_id=3)
    job = await queue.pop_job()
    assert job == (2, 2, 200.0)
...
async def test_recover_stale_requeues_expired(queue) -> None:
    ...
    await queue._client.zadd(queue._settings.processing_key, {"proc:9:pf:1": time.time() - 100})
    await queue.recover_stale()
    assert await queue._client.zscore(queue._settings.jobs_key, "proc:9:pf:1") == 7.0
    assert await queue._client.zscore(queue._settings.processing_key, "proc:9:pf:1") is None


async def test_recover_stale_keeps_fresh(queue) -> None:
    assert queue._client is not None
    await queue._client.zadd(queue._settings.processing_key, {"proc:5:pf:1": time.time()})
    await queue.recover_stale()
    assert await queue._client.zscore(queue._settings.jobs_key, "proc:5:pf:1") is None
    assert await queue._client.zscore(queue._settings.processing_key, "proc:5:pf:1") is not None


async def test_recover_preserves_claimed_priority(queue) -> None:
    await queue.claim_processing(9, 1, 100.0)
    assert queue._client is not None
    # делаем аренду просроченной
    await queue._client.zadd(queue._settings.processing_key, {"proc:9:pf:1": time.time() - 100})
    await queue.recover_stale()
    assert await queue._client.zscore(queue._settings.jobs_key, "proc:9:pf:1") == 100.0
    assert await queue._client.zscore(queue._settings.processing_key, "proc:9:pf:1") is None
    assert await queue._client.hget(queue._settings.processing_meta_key, "proc:9:pf:1") is None
