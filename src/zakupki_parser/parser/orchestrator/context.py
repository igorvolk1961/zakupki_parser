...
class ProfileRunContext:
    ...
    # Сужение серверных критериев ОКПД2 ЖИВОГО обхода (Stage D плана «индекс как
    # основной механизм discovery», ``Scheduler._split_ctxs_for_index_routing``):
    # None — обход строится по ``profile.okpd_codes`` как обычно (весь диапазон
    # профиля). Список — используется ВМЕСТО ``profile.okpd_codes`` ТОЛЬКО для
    # построения критериев обхода (``Orchestrator._build_units``); сам ``profile``
    # не трогается, поэтому матчинг/скоринг/индексная синхронизация по-прежнему
    # видят полный исходный диапазон профиля. Нужен для профиля, частично
    # покрытого фоновой индексацией: коды, уже покрытые индексом, синхронизируются
    # из БД без обращения к площадке (``_sync_profiles_via_index``), а живой обход
    # запрашивается только по ОСТАЛЬНЫМ (не покрытым) кодам — не по всему профилю.
    crawl_okpd_codes: list[str] | None = None
