...
class Orchestrator(
    ...
    def __init__(
        ...
        # Дозагрузка деталей площадки отдельным best-effort проходом (BR-08): записи,
        # сохранённые на уровне списка без платформенных деталей, собираются ПОСЛЕ
        # постановки в очередь скоринга, чтобы сбой API деталей (напр. mos.ru 402)
        # не блокировал скоринг и не валил проход всей площадки.
        self._pending_details: list[dict[str, Any]] = []
    ...
    async def run(
        ...
        *,
        ...
        # Досборка деталей площадки ПОСЛЕ постановки в очередь скоринга (BR-08):
        # best-effort — сбой деталей (напр. mos.ru 402) не роняет проход и не влияет
        # на уже поставленный скоринг.
