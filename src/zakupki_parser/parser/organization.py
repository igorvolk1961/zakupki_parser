...
platforms.<id>.organization``:
  ...
  - ``inn_page_selector`` — ИНН извлекается со страницы организации (mos.ru, ЕИС 44-ФЗ).
...
async def _inn_from_org_page(
    page: Page, platform: PlatformDom, customer_link: str, selector: str
) -> str | None:
