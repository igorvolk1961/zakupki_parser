...
class SearchFilterConfig(BaseModel):
    ...
    keyword_query_param: str | None = Field(
        default=None,
        description=(
            "имя query-параметра серверной предфильтрации по ключевым словам профиля "
            "(например f_keyword у b2b-center). Если задан и у поискового обхода есть "
            "ключевые слова — в запрос подставляется строка `(фраза) или слово`, "
            "собранная из позитивных слов профиля: выражения-проксимити `(фраза)~N` "
            "приводятся к `(фраза)` (скобки остаются, `~N` убирается). "
            "Сервер сужает выдачу, итоговая фильтрация по словам остаётся клиентской (R9). "
            "Применяется для площадок без серверного ОКПД2/иных критериев (например b2b-center)."
        ),
    )
