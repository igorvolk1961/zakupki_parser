...
class ParserConfig(BaseModel):
    ...
    details_batch: int = Field(
        default=50,
        ge=1,
        description=(
            "максимальное число закупок за проход для досборки деталей площадки ПОСЛЕ "
            "получения результата скоринга (BR-08): закупки, получившие fit_score, но ещё "
            "не получившие детали, обрабатываются партиями по этому лимиту"
        ),
    )
    max_concurrent_platforms: int = Field(
        default=2,
        ge=1,
        description=(
            "максимум одновременно обрабатываемых площадок (параллельные asyncio-задачи "
            "планировщика, R5): задержка/backoff одной площадки не блокирует другие. "
            "1 — строго последовательно (прежнее поведение)"
        ),
    )
    max_concurrent_per_domain: int = Field(
        ...
        ge=1,
        description=(
            "максимум одновременно обрабатываемых площадок ОДНОГО домена/бэкенда "
            "(``domain_group`` в configs/dom/<площадка>.yaml): 44-ФЗ и 223-ФЗ одного сайта "
            "(например lot-online) не запускаются параллельно — общий IP/антибот и "
            "rate-limit не перегружаются. 1 — площадки одного домена строго "
            "последовательно"
        ),
    )
