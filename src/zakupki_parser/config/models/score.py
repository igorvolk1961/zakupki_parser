...
class ScoreConfig(BaseModel):
    ...
    # TTL «постановки в очередь»: неотскорингованная пара (закупка, профиль), чья
    # метка scoring_queued_at старше этого порога, снова ставится в очередь recovery
    ...
    recovery_ttl_seconds: float = Field(
        ...
        ge=0,
        description=(
            "TTL (сек) для повторной постановки в очередь неотскорингованных пар "
            "(закупка, профиль): scoring_queued_at старше порога -> пара снова "
            "ставится в очередь (задание потеряно). 0 — выключено"
        ),
    )
