...
class IndexingConfig(_BaseConfig):
    ...
    Системный «индексный» профиль (без владельца-пользователя) с этими кодами ОКПД2
    и пустыми ключевыми словами включается в обычный обход площадок наравне с
    пользовательскими профилями (см. ``Scheduler._gather_profile_ctxs``): по
    семантике «пустой список ключевых слов — фильтра нет» сохраняются ВСЕ закупки
    этого диапазона, независимо от того, совпали ли они с чьими-то ключевыми
    словами. Индексируются описания (``search_tsv``) и текст документов —
    ``indexing_service``. Роутинг «горячего» пересбора (``scheduler.py``) использует
    этот же список для решения «мгновенно из индекса vs живой краулинг».
    """

    enabled: bool = Field(
        default=False,
        description=(
            "включить фоновую индексацию (иначе — системный профиль в обход не добавляется)"
        ),
    )
    okpd2_prefixes: list[str] = Field(
        default_factory=list,
        description=(
            "конфигурируемый список кодов ОКПД2 (префиксы иерархии, напр. '62', '38'), "
            "в границах которого сохраняются ВСЕ закупки и работает фоновая индексация"
        ),
    )
    excluded_platforms: list[str] = Field(
        default_factory=lambda: ["b2b_center"],
        description=(
            "площадки без серверной фильтрации по ОКПД2 (см. docs/adr.md) — индексный "
...
class ServiceConfig(_BaseConfig):
    ...
    deduplicate_requests: bool = Field(
        default=True,
        description=(
            "объединять одинаковые обходы одной площадки (совпадающие коды ОКПД2, "
            "НМЦК-диапазон, состояние) в один запрос — результат веерно фильтруется "
            "под каждый профиль. По умолчанию включено: меньше нагрузка на площадку и "
            "вероятность блокировок, но статистика обхода общая на группу профилей. "
            "Когда стоит отключать — см. docs/profile-crawling.md"
        ),
    )
