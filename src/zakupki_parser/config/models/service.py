...
class StopConditions(_BaseConfig):
    ...
    keyword_context_required: bool = Field(
        default=False,
        description=(
            "при поиске по ключевым словам: пропускать закупку, если в описании (subject) "
            "ни одно ключевое слово не встречается как отдельное слово или перед дефисом "
            "(регистронезависимо); отсекает ложные совпадения, когда слово входит в другое "
            "(например, 'ИИ' в 'инвестиции'); пустой subject при включённом флаге считается "
            "критическим дефектом записи и также пропускается"
        ),
    )
    keyword_context_regexes: dict[str, str] = Field(
        default_factory=dict,
        description=(
            "явные regex-паттерны для keyword_context_required: ключ — ключевое слово из "
            "search_criteria.keywords, значение — паттерн, проверяемый в описании "
            "(регистронезависимо) вместо стандартного «отдельное слово». Нужно для слов, "
            "которым подходят разные морфологические формы (например, 'автоматизация': "
            "'автоматизаци\\\\w*' найдёт и 'автоматизации'). Паттерн применяется как есть "
            "(границы слова — на ваше усмотрение, напр. '(?<!\\w)автоматизаци\\\\w*'). "
            "Слова без паттерна — строго отдельное слово или перед дефисом"
        ),
    )
