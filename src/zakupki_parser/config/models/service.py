...
class StopConditions(_BaseConfig):
    ...
    keyword_context_required: bool = Field(
        default=False,
        description=(
            "стоп-условие по описанию (subject) для поисков по ключевым словам: "
            "пропускать закупку, если ни одно проверяемое ключевое слово не "
            "встречается в описании; применяется ТОЛЬКО к словам, для которых задан "
            "паттерн в keyword_context_regexes (слово без паттерна не проверяется); "
            "пустой subject при наличии проверяемых слов — критический дефект, "
            "закупка также пропускается"
        ),
    )
    keyword_context_regexes: dict[str, str] = Field(
        default_factory=dict,
        description=(
            "regex-паттерны для keyword_context_required: ключ — ключевое слово из "
            "search_criteria.keywords, значение — паттерн, проверяемый в описании "
            "(регистронезависимо). Только слова с паттерном участвуют в стоп-условии: "
            "слово без паттерна не фильтрует. Например, 'ИИ': '(?<!\\w)ИИ(?!\\w)' — "
            "отдельное слово/перед дефисом (отсекает 'инвестиции'); 'автоматизация': "
            "'автоматизаци\\\\w*' — словоформы 'автоматизации'. Паттерн применяется "
            "как есть (границы слова — на ваше усмотрение)"
        ),
    )
    exclusion_words_present: bool = Field(
        default=False,
        description=(
            "стоп-условие по словам-исключениям активного клиентского профиля "
            "(client_profiles.exclusion_words): пропускать закупку, если в описании "
            "(subject) встречается любое слово-исключение (по границе слова, "
            "регистронезависимо). Примеры из обсуждения: нет медицинских компетенций — "
            "слово 'медицинский'; при слове 'искусственный' — 'ель', 'лед', 'алмаз'. "
            "Список слов задаётся в профиле клиента, флаг включает проверку"
        ),
    )
