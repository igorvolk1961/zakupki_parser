...
class SearchCriteria(_BaseConfig):
    ...
    keywords: list[str] = Field(
        default_factory=list,
        description=(
            "DEPRECATED: ключевые слова НЕ передаются на площадку (R9) и не берутся "
            "из этого конфига. Слова профиля хранятся в таблице ``keywords`` "
            "(канонический источник; сид — ``zp seed-profile`` из ``data/profile.md``) "
            "и применяются клиентской пост-фильтрацией до записи в БД. Поле оставлено "
            "для совместимости веб-редактора конфигурации и модуля построения "
            "URL-запросов (query.py)"
        ),
    )
