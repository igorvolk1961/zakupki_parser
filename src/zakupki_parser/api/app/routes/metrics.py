...
def _load_tz(name: str | None) -> Any:
    """Часовой пояс из строки конфига (например, ``Europe/Berlin``); при ошибке — UTC.
...
def _rec_order(record: dict[str, Any]) -> tuple[bool, int, str]:
    """Порядок для группировки: сначала итерация цикла (по номеру), затем записи без неё.
    ...
    строка журнала несла свою площадку и итерацию. Записи без итерации (ручной
    запуск вне цикла планировщика) — в конец.
    """
...
def build_metrics_journal(
    ...
    *,
    ...
    for record in sorted(records, key=lambda r: (_rec_order(r), r["created_at"] or "")):
        ...
        # Граница батча: сменилась пара (итерация, площадка) — либо у записей
        # без итерации (ручной запуск) зазор по времени превысил паузу цикла, либо у любых записей
