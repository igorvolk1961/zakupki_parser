...
def build_context(state: AppState) -> ApiContext:
    ...
    async def _active_context(user: User | None) -> tuple[User | None, Profile | None]:
        ...
        Если у пользователя нет ни одного профиля, но роль даёт право на профиль,
        он создаётся (``ensure_default_profile``) — легаси-аккаунты чинятся на
        первом запросе. Если у пользователя есть хотя бы один профиль, активный
        контекст разрешается всегда (FR-1.3): ``get_active_profile`` возвращает
        единственный активный либо первый по id, включая выключенные профили
        (активность не зависит от участия в мониторинге). 409 — только когда
        профилей нет вовсе и роль не даёт права на профиль.
        """
        if user is None:
            return None, None
        profile = await _repo().get_active_profile(user.id)
        if profile is None:
            # Само-лечение легаси-аккаунтов (BR-07): у роли user/analyst профиль
            # должен быть. Создаём default-профиль только когда профилей нет
            # вовсе (аккаунт создан до мультитенантности): если профили есть, но
            # отключены, пользователь должен выбрать/включить один сам — иначе
            # молчаливое создание вернуло бы его в обход парсера. Для ролей без
            # права на профиль (admin/devops) вернётся None.
            rows, _ = await _repo().list_profiles(user.id, limit=1)
            if not rows:
                profile = await _repo().ensure_default_profile(user.id, user.roles)
        if profile is None:
            raise HTTPException(
                status_code=409,
                detail="У пользователя нет доступного профиля: создайте или включите профиль",
            )
        return user, profile
