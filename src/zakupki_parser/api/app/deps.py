...
def build_context(state: AppState) -> ApiContext:
    ...
    async def _active_context(user: User | None) -> tuple[User | None, Profile | None]:
        ...
        Если у пользователя есть хотя бы один профиль, активный
        ...
        профилей нет вовсе (профиль создаётся вместе с пользователем и при
        выдаче роли user/analyst).
        """
        if user is None:
            return None, None
    ...
    async def _scoring_embeddings_enabled(profile: Profile) -> bool:
        ...
        (options.py: ``scoring_embeddings``). Профиль без владельца опцию не
        получает.
        """
