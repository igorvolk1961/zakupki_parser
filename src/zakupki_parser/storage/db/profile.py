...
class Profile(Base):
    ...
    # Максимально допустимое расстояние от центра ближайшего целевого региона, км
    # (несколько регионов — расстояние до ближайшего из них). Считается ТОЛЬКО на
    # этапе анализа (analysis_service, внешний сервис) — решение требует
    # гео-координат, которых у парсера нет. Готовый вердикт (rag_report['geo'])
    # затем применяется как пост-фильтрация в POST /api/procurements/{id}/score
    # (routes/procurements.py) — так же, как строковый регион и ключевые слова.
...
class Keyword(Base):
    ...
    Слова задаются в профиле через веб-редактор или импорт JSON-файла.
    """
...
class ExperienceConfirmationType(Base):
    ...
    ``code`` — стабильный ключ: ``platform`` (через электронную площадку, ПП 2571),
...
class ProfileExperience(Base):
    ...
    подтверждение через площадку (ПП 2571), сканы договоров/актов, выписка из
