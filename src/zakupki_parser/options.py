...
PAID_OPTIONS: tuple[OptionDef, ...] = (
    ...
    OptionDef(
        key="competencies_from_url",
        title="Заполнение компетенций по сайту (LLM)",
        description=(
            "Кнопка «Заполнить по URL» в редакторе профиля: сервис скачивает сайт "
            "поставщика и просит языковую модель собрать по нему компетенции."
        ),
        group=GROUP_PAID,
    ),
    OptionDef(
...
# компетенций по сайту — разовые/выборочные операции по явному запросу
