...
* вынос завершающего маркера («не установлено», «не применяется(-ются)»,
  «не предоставляется(-ются)», «не требуется(-ются)», «не используется(-ются)»,
  «не задано/-а/-ы/не задаётся(-ются)») в отдельную ячейку той же строки.
"""
...
# («не требуется»/«не требуются», «не предоставляется»/«не предоставляются»,
# «не используется»/«не используются», «не задано»/«не задается» и т.п.).
_MARKER_PHRASE = (
    r"не\s+(?:установлен[а-яё]*|применя(?:ется|ются)|"
    r"предоставля(?:ется|ются)|требу(?:ется|ются)|"
    r"использу(?:ется|ются)|зада(?:ется|ётся|ются|но|на|ны|н))"
)
