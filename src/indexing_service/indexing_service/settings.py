...
class Settings(BaseSettings):
    ...
    # документов, см. риски плана индексации. Лимит проверяется ПОСЛЕ добавления
    # очередного файла целиком (см. worker.py::_compute_payload) — ни один файл
    # не обрезается на середине, только последующие файлы пропускаются. Значение
    # с большим запасом, чтобы редкий крупный ТЗ-файл не "вытолкнул" следующие
    # файлы закупки за пределы бюджета.
