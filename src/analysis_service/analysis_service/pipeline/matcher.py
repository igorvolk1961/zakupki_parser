"""Сопоставление лицензий требования ТЗ с лицензиями профиля (BR-03/BR-04).

Детерминированные правила, без LLM: вид лицензии/допуска определяется из
``requirements_json`` (LLM-поле ``data``, если заполнено, иначе лексический
fallback по тексту требования) и сопоставляется с названиями лицензий профиля.
Нераспознанный вид лицензии не отсеивает закупку — отчёт просто помечает его
как требующий проверки (recall-over-precision, решение за тендерологом).
"""
