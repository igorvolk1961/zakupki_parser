...
class ReportFieldValue(BaseModel):
    ...
    # Уровень барьера при несоответствии (match=False) — копия из профиля
    # (``report_fields[].severity``): ``block`` — закупка не принята, ``soft`` —
    # снижает P(win), None — не барьер. Для удобства фронта/Excel.
