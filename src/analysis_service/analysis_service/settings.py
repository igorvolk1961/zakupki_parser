...
class Settings(BaseSettings):
    ...
    # Вопросы профиля ищут ответ по ВСЕМ документам закупки (не только по файлу,
    # определённому как ТЗ, — тот же охват, что у «Требований к участнику»,
    # enumerate_document_refs), поэтому нужны те же защитные лимиты, что и у
    # фоновой индексации (indexing_service.settings), иначе закупка с большим
    # числом крупных вложений неограниченно раздувает стоимость эмбеддингов.
    max_files_per_procurement: int = Field(
        default=20, ge=1, description="макс. число документов закупки, обрабатываемых на вопросы"
    )
    max_document_chars: int = Field(
        default=2_000_000, ge=1000, description="макс. суммарный текст документов закупки, символов"
    )
    # Конструктор отчётных полей (FR-12.2): top_k выше, чем у вопросов —
    # табличные поля (цена, объём) чаще требуют больше окружающего контекста,
    ...
    # Поле-список (например, коды ФККО): LLM видит только top_k фрагментов и
    # даёт образцы, полноту обеспечивает код. Участок текста вокруг найденных
    # значений (от первого до последнего ± запас) повторно отдаётся LLM, если
    # она его не видела целиком.
    list_span_margin: int = Field(
        default=1500, ge=0, description="запас вокруг найденных значений списка, символов"
    )
    list_span_max: int = Field(
        default=30_000, ge=1000, description="макс. длина участка для дозаполнения списка"
    )
