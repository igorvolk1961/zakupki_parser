...
1. Подпишитесь на события бота через Long Polling или Webhook
   (`POST /subscriptions`), либо опрашивайте `GET /updates`.
2. Напишите в канал любое сообщение или добавьте бота — придут события
   `bot_added` / `bot_started`, в объекте `Update` которых будет `chat_id`.
