...
| `etpgpb_223` | ЭТП ГПБ | 223-ФЗ | ✅ список+детали+файлы+ИНН (снимок) | готов к тесту | выкл |
| `etpgpb_44` | ЭТП ГПБ | 44-ФЗ | ✅ список+детали+файлы+ИНН (та же VESNA-структура) | готов к тесту | выкл |
...
| etpgpb_223 | Vue SPA | `procedure[stage][0]`+`procedure[okpd][N]`+`sort`+`per`+`page` | ✅ `procedure[okpd][0]=62.02` (raw, префиксный матчинг) | ❌ (`procedure[name]` не фильтрует) | дата `by_published_desc` | да |
| etpgpb_44 | Vue SPA | то же | ✅ | ❌ | дата | да |
