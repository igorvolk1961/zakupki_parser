...
На части площадок (lot-online 223, b2b-center) ключевые слова можно объединить
**союзом «или» в одном запросе**, а не перебирать по-одному. Для этого:
...
- `keywords_quote_phrases: true` — многословные фразы заключаются в двойные кавычки
  (например, `"искусственный интеллект" или автоматизация`), чтобы фраза искалась точно.
По умолчанию `keywords_separator` — пробел, `keywords_quote_phrases` — false
(поведение не меняется для площадок, где союз не поддерживается).
