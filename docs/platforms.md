...
Обновлено: 2026-08-29. Конфиги площадок — в `configs/dom/<platform_id>.yaml`
...
| `roseltorg_223fz` | Росэлторг | 223-ФЗ | ✅ список (первая страница) + детали: ОКПД2 (JSON-LD); полный Chromium | частично | выкл |
| `fabrikant` | Фабрикант | закупки (44/коммерческие) | ✅ список+детали (data-slot, снимок); ОКПД2/файлы — TODO (client-rendered) | готов к тесту | выкл |
...
### Регион закупки (общее правило и статус по площадкам)
Регион закупки определяется по приоритету: **место поставки**, а если оно не указано —
**регион/местонахождение заказчика**. Правило одинаково для всех площадок (2026-09-04).
Хранится строкой в `procurements.region`, фильтр профиля — клиентский (шаблоны R9),
...
- **tender.lot-online (223-ФЗ)** — `customerOkato`/`regionOkato` в реестре indexer и в
  `commonInfo` деталей, ✅ уровень списка + детали.
- **ЕИС 44/223-ФЗ** — блок «Регион» общей карточки (детали), ✅ детали (44-ФЗ настроен,
  223-ФЗ добавлен по аналогии, селектор копии 44-ФЗ).
...
- **lot-online (gz, 44-ФЗ)** — в реестре `/etp_back/procedure/list` и в `lotInfo` JSON-RPC
  ...
  (common, поле-метка «Место поставки»). Регион из DOM берётся ТОЛЬКО по явному
  региональному запросу профиля (`region_on_demand_dom`: при скоринге, если профиль
  задал target_regions/расстояние, а регион неизвестен, открывается common-страница).
- **b2b_center** — «Адрес поставки / оказания услуг» на новой SSR-детали market-next
  (секция `.delivery-address-list`, напр. «…г. Санкт-Петербург, поселок Понтонный…»)
  виден анонимно — ✅ детали (переменная region в b2b_center.yaml). Регион-фильтры
  поиска — только зарегистрированным.
