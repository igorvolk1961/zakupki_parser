...
| zakupki_gov_44fz | сервер | query-параметры | ✅ `okpd2Ids`+`okpd2IdsWithNested` | — (тематику задаёт ОКПД2) | дата `UPDATE_DATE` | да |
| zakupki_gov_223fz | сервер | query-параметры | ✅ (через lot-list) | — | дата `UPDATE_DATE` | да |
...
| fabrikant | Next.js SPA | — (в URL не отражается) | ❌ | пост-фильтр (`post_filter_keywords`) | релевантность | нет |
| lot_online_223 | Angular SPA | — (в URL не отражается) | ❌ | пост-фильтр | релевантность | нет |
...
- **Клиентский пост-фильтр по ключевым словам** `post_filter_keywords` — для SPA, где серверный
  текстовый поиск в URL не отражается (fabrikant, lot_online_223).
...
- **ЕИС** — текстовый поиск `searchString` и сортировка `sortBy=RELEVANCE` не используются
  (тематику задаёт ОКПД2); включение — отдельное решение.
