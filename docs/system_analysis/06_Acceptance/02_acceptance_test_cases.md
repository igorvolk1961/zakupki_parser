...
### TC-4.2 RAG-вердикт «не установлено» (US-4.3, BR-04)
- **Given**: вопрос профиля про реестр Минпромторга; ТЗ с запретом иностранной продукции и пометкой «не установлено».
- **When**: RAG-анализ завершён.
- **Then**: вердикт `no_stop_condition` или `soft` (не `absolute`); цитата в `excerpt`.

### TC-4.3 RAG-вердикт жёсткого требования (US-4.2/4.4)
- **Given**: ТЗ с обязательной лицензией, которой нет у компании.
- **When**: RAG-анализ завершён.
- **Then**: вердикт `absolute` (жёсткий запрет) с цитатой.

### TC-4.4 Сбой LLM не роняет анализ (NFR-FT-2, FR-4.2)
- **Given**: LLM недоступен.
- **When**: анализ закупки.
- **Then**: задание не падает; вердикт с пометкой «LLM-верификация не выполнена (сбой)» или `tz_found=false`.
