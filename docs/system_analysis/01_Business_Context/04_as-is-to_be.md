...
- Вручную проверить наличие жестких барьеров: требование подтвержденного опыта (ПП № 2571), нюансы реестра Минпромторга («не установлено»), наличие лицензий.
...
6. **Шаг 6 — Глубокая проверка документов и обновление карточки** (Эпик 4, v2.0):
   Система анализирует все документы закупки: извлекает значения отчётных полей
   профиля и проверяет их условия (в том числе по тексту сайта — например, коды
   ФККО по лицензии поставщика), находит требования к участнику (лицензии, опыт,
   реестр Минпромторга, соисполнители), считает расстояние до центра региона и
   выносит вердикт: жёсткий барьер авто-отклоняет закупку с причиной, мягкий —
   снижает её P(win) (опыт — по способу подтверждения, BR-03).
