...
- **Analysis Service** — on-demand анализ всех документов закупки: требования к
  участнику детерминированно (лицензии со сводкой по лицензиям профиля, опыт,
  Минпромторг, соисполнители), значения отчётных полей профиля (LLM) и проверка их
  условий кодом — в том числе по тексту сайта-источника из объектного хранилища
  (FR-13.8); расстояние до центра региона; вердикт и авто-отклонение.
- **Сайты-источники** — сбор текста сайта по всем страницам пагинации (браузер в
  API-процессе, SSRF-проверка каждого запроса) → объектное хранилище S3/MinIO.
