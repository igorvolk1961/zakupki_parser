...
>
> ⚠️ **Источник правды связок «требование ↔ код ↔ тест ↔ стейкхолдер ↔ ADR»** —
> `../traceability/requirements-registry.yaml` (машиночитаемый, на базе этого реестра
> работает `scripts/check_traceability.py`). Статусы в этой матрице — укрупнённые
> (✅/🟡/❌); точный статус (`implemented|partial|planned|out-of-scope`), код-модули и
> тест-файлы — в реестре.
...
| Операционная работа | 1, 6 | FR-10.1–10.8 (инструментальные, владельцы — DevOps/SRE, Сист.админ, Аналитик) | ✅ |
