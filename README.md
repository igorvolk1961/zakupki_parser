...
Парсер площадок закупок (zakupki.mos.example, ЕИС и др.) с веб-интерфейсом.
...
региона (`zakupki.mos.example`), ЕИС (`zakupki.gov.example`, 44-ФЗ/223-ФЗ) и коммерческие ЭТП
...
- Параллельная обработка площадок (`max_concurrent_platforms`, дефолт 2): каждая
  площадка — отдельная asyncio-задача, задержка/backoff одной не блокирует другие.
  Площадки одного домена/бэкенда (`domain_group`, напр. 44-ФЗ/223-ФЗ одного сайта)
  ограничены дополнительно через `max_concurrent_per_domain` (дефолт 1): идут строго
  последовательно, чтобы не перегружать общий IP/антибот/rate-limit сайта.
...
Заполнить профиль — через веб-интерфейс (редактор профиля) либо импортом единого
JSON-файла (`POST /api/clients/import`, схема `zakupki-profile`; поля профиля и
компетенции-подобъект внутри). Ключевые слова записываются в таблицу `keywords`
...
- **MAX** — работает из страны без прокси: рекомендован как основной способ.
- **Telegram** — требует доступа к `api.telegram.org` (VPN/прокси). Важно:
  при включённом VPN ЕИС (`zakupki.gov.example`) может быть недоступен, поэтому для
...
## Конфигурация
- `config_parser.yaml` — браузер и антиблок-меры, а также параллельность обхода
  площадок (`max_concurrent_platforms`: число одновременно обрабатываемых площадок,
  `max_concurrent_per_domain`: лимит площадок одного домена, напр. 44-ФЗ/223-ФЗ;
  1 — строго последовательно).
...
- `config_service.yaml` — **аналитические** настройки: список сайтов, порог дат,
  ...
  (ОКПД2, НМЦК, состояние) задаются в ПРОФИЛЕ (таблица `profiles`), а не в этом
  конфиге.
