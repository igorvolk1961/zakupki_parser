...
- **Параметры мониторинга** (роль `analyst`) — форма по схеме для аналитических
  параметров `config_service.yaml` + «Расширенный режим» с сырым YAML.
...
- **Конфигурация / Управление Логи / Логи / Парсер** (роль `devops`) — форма для
  `config_ops.yaml` и `config_log.yaml` (+ расширенный режим YAML), просмотр хвоста
...
- **Парсер** (роль `devops`) — панель управления парсером (Запустить/Остановить,
  очистка БД).
- Секреты (auth.secret, токены уведомлений) в конфигах через API не редактируются
  — они берутся из env. Изменения конфигов применяются при следующем запуске.
