...
- [ ] **lot_online (44/223): детальные страницы (SPA API)** — ОКПД2/цена/файлы отдаются через
      внутренний JSON-RPC `POST /etp_back/api/get` (`manager`/`entity`/`fields`/`rules`), в DOM не
      рендерятся. Нужен отдельный механизм API-извлечения (или ожидание рендера после действий).
