...
- [x] **Верификация селекторов ЭТП ГПБ (etpgpb)** — снимки списка/детали/организации
      ...
      (`div.productCard__title a`); `procedure[name]` НЕ фильтрует (keywords убраны).
      ОКПД2-фильтр: движок — `raw_array` (код как есть в `procedure[okpd][N]`), сервер матчит
      по префиксу (вложенные коды включаются автоматически, дерево не нужно).
      Площадка не различает 44/223 — одна площадка `etpgpb` (etpgpb_223 удалён, etpgpb_44
      переименован в etpgpb).
