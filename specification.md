...
6. **Для каждой записи**:
   ...
   **Детали площадки дособираются отдельным best-effort проходом ПОСЛЕ получения
   результата скоринга (BR-08).** Внешний скоринг (Fit) возвращает результат через
   `POST /api/procurements/{id}/score`; только для закупок с записанным `fit_score`
   планировщик в следующих проходах догружает платформенные детали
   (`find_scored_without_details`, контекст `detail_api`): ОКПД2, метаданные файлов
   (`files_json`, парсер НЕ скачивает файлы — сохраняются пары `{name, url}`),
   ИНН заказчика (ADR-4), статус/НМЦК. Сбой деталей (например, HTTP 402 от API
   mos.ru при исчерпании квоты) НЕ роняет проход площадки и НЕ блокирует скоринг —
   карточка остаётся на уровне списка, досборка повторится в следующем цикле.
