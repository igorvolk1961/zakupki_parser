...
3. **Применение фильтров**. Поддерживается два механизма:
   ...
   **Прочие критерии** — диапазон НМЦК (`nmck_min`/`nmck_max`), коды ОКПД2
   (`okpd_codes`) задаются в ПРОФИЛЕ (таблица `profiles`; сид — `docs/references/profile.md`,
...
- **R8** — сид таблицы `keywords` из `docs/references/profile.md` (скрипт `zp seed-profile`).
...
| 2 | MVP | Профили фильтрации (Эпик 1): per-user CRUD, сид `keywords` из `docs/references/profile.md` | `plans/plan.md` |
