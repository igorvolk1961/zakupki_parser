...
6. **Для каждой записи**:
   ...
   6. если `download_files: true` — скачивание файлов в хранилище
      (`config_service.yaml -> storage`: `local` — `documents/<номер>/`, или `s3` —
      MinIO/совместимое объектное хранилище); при `download_technical_spec_only: true`
      скачиваются только файлы, в имени которых есть хотя бы одно из
      `technical_spec_keywords` (например, только «техническое задание»).
      **Защита**: включённый флаг при пустых `technical_spec_keywords` — ошибка
      валидации конфига при загрузке;
      в БД пишется ссылка `technical_spec_url` (URL объекта в MinIO или путь
      в `documents`), а не бинарный файл; техническое задание **сохраняется**
      (исключается из удаления);
