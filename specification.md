...
3. **Применение фильтров**. Поддерживается два механизма:
   ...
   **ОКПД2**: фильтр по кодам ОКПД2 задаётся человекочитаемыми кодами
   (`search.okpd_codes`, например `["62", "63"]`) и резолвится в пути узлов дерева
   площадки (`okpdPaths`) через маппинг `search.okpd_tree_file`
   (`docs/codes/okpd2_tree.json`). Выбор ветви-предка включает всех потомков.
   Код, отсутствующий в маппинге, пропускается с предупреждением.
