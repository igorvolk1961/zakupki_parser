...
| `dom/` (`<platform_id>.yaml`) | URL площадки, имена переменных, селекторы контейнеров и значений, а также селекторы сортировки и фильтров (блоки `sort`/`filters`). |
...
3. **Применение фильтров**. Поддерживается два механизма:
   ...
   **Прочие критерии** (`config_service.yaml -> search_criteria`): диапазон НМЦК
   ...
   ключевые слова на ЕИС из поиска убраны (тематику задаёт ОКПД2); на площадках, где
   серверный поиск по словам отражается в URL (mos `nameLike`, B2B-Center `keywords=`),
   ключевые слова подставляются через `criteria_map.keywords`. Для SPA без серверного
   текстового поиска (Фабрикант, lot-online 223-ФЗ) задаётся клиентский пост-фильтр
   (`list_config.post_filter_keywords=true`): записи, в `subject`/`number` которых нет
   ни одного из `search_criteria.keywords`, отсекаются (пропуск контейнера).
