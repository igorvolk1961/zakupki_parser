...
| R9 Ключевые слова — только клиентская пост-фильтрация до записи; сервер — только ОКПД2 (+ обход «без кода»; без позитивных слов обход «без кода» пропускается с логом) | ✅ реализовано (этап 3): серверных слов нет (`orchestrator`), клиентская фильтрация `parser/filtering.py`, `no_code_search` | `orchestrator.py`, `filtering.py` | 3 |
